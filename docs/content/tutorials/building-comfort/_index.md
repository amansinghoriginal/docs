...
To launch the dashboard, open a new terminal and use the following commands
  inside the `dashboard` directory. You will get a prompt to use the next port
  (3001) as we have the Control Panel running on port 3000. Select 'Y' for yes.
...
This should launch the dashboard on port 3001.
...
Check if the URLs for both control-panel and dashboard frame are correct in the file at `dashboard/index.html`.
Update the ports if using different ones.
...
This should expose a new port and open the corresponding URL created by Github
  codespaces. You can go to PORTS tab and open the URL for `5500`.
...
Open this file path in your browser after replacing the placeholder with path
  to where you cloned the learning repo:

```
file:///<path-to-learning-repo>/apps/building-comfort/dashboard/index.html
```
...
[Synthetic Relationships](#synthetic-relationships) in Drasi also helped us express the hierarchy between Rooms, Floors and Buildings.
...
Drasi can represent one or more existing data sources as Graphs which can be linked together using [Synthetic Relationships](#synthetic-relationships).
